...
def sha_inf(model,params,units,dir,n,B,eta=2):
    def log_eta(x):
...
def hyperband_inf(model,runtime,units,dir,params,eta,starting_b=1,min_unit=1,min_arms=64):
    # input t in minutes
    t_0 = time.time()
    ...
    results_dict={}
    time_test=[]
    while minutes(time.time())< runtime and k <2:
        l=0
        B=starting_b*2**k
        ...
        while l>=0 and minutes(time.time())< runtime:
            n=eta**l
            if B/n/max(1,numpy.ceil(log_eta(n))+1)>min_unit and n >=min_arms:
                print 'running sha'
                print 's=%d, n=%d' %(l,n)
                arms,result = sha_inf(model,params,units,dir, n,B,eta)
...
def main(argv):
    ...
    if model=='cifar10':
        ...
        hyperband_inf(obj,720,'iter',dir,params,4,starting_b=120000,min_unit=100,min_arms=4)
